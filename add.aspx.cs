...
public partial class Test_List_add : Page
{
    ...
    private void InsertData()
    {
        ...
        search = new SqlCommand("Select * From Teacher Where TID=@TID;", conn);
        search.Parameters.Add("@TID", SqlDbType.Int).Value = TID.Text;

        if (search.ExecuteScalar() != null) {
            Response.Write("<script>alert('Teacher is exisit!')</script>");
        }
        else {
            try
            {
